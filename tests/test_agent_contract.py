...
def _normalize_agent_data(value):
    if isinstance(value, dict):
        normalized = {
            key: _normalize_agent_data(item)
            for key, item in value.items()
            if key not in {"filepath", "revision"}
        }
        return normalized
    if isinstance(value, list):
        return [_normalize_agent_data(item) for item in value]
    return value


def test_agent_exact_store_and_legacy_adapter_are_semantically_equivalent(tmp_path: Path) -> None:
    exact_parent = tmp_path / "exact"
    exact_parent.mkdir()
    project_id = "legacy-project"
    id_prefix = "LEG"
    exact = _make_store(exact_parent, project_id=project_id, id_prefix=id_prefix)
    legacy_parent = tmp_path / project_id
    legacy = legacy_parent / "backlog"
    legacy_parent.mkdir()
    legacy.mkdir()
    (legacy / "INDEX.md").write_bytes((exact / "INDEX.md").read_bytes())
    (legacy / "items").mkdir()
    _write_item(exact, "LEG-001", project_id=project_id)
    _write_item(legacy, "LEG-001", project_id=project_id)
    assert not (legacy / "backlog.json").exists()
    runner = CliRunner()

    def invoke(command: list[str]) -> tuple[dict, dict]:
        return (
            _json(runner, ["--store", str(exact), *command]),
            _json(runner, ["--target", str(legacy_parent), *command]),
        )

    for command in (["list", "--json"], ["show", "LEG-001", "--json"]):
        exact_payload, legacy_payload = invoke(command)
        assert _normalize_agent_data(exact_payload) == _normalize_agent_data(legacy_payload)

    exact_add, legacy_add = invoke(["add", "--title", "Added", "--category", "feature", "--priority", "P1", "--json"])
    assert _normalize_agent_data(exact_add) == _normalize_agent_data(legacy_add)
    exact_revision = exact_add["data"]["revision"]
    legacy_revision = legacy_add["data"]["revision"]
    exact_update = _json(
        runner,
        [
            "--store",
    ...
    assert _normalize_agent_data(exact_update) == _normalize_agent_data(legacy_update)

    exact_next, legacy_next = invoke(["next", "--json"])
    assert _normalize_agent_data(exact_next) == _normalize_agent_data(legacy_next)
